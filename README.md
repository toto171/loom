...
loom sbom     spec/vehicle.example.yaml                                 # vehicle + per-module + toolchain SBOMs (no sim)
loom deploy   spec/vehicle.example.yaml                                 # Eclipse Ankaios workload manifest for the composition

uvicorn dashboard.app:app          # web dashboard at http://127.0.0.1:8000
pytest                             # 147 tests (M0–M6)
```

Each `loom run` writes `runs/<id>/` — trace, report, violations, an aggregate CycloneDX
vehicle SBOM, one per-module SBOM (each contract's `sbomRef`), the transitive toolchain SBOM,
and a GSN assurance case. `loom sbom` emits just the SBOMs without a sim; `loom deploy`
generates an Ankaios deployment manifest. See the [CLI reference](docs/cli.md).
...
The full roadmap is implemented and green (**147 tests**). Highlights, each with automated
...
- **M5** — assurance generation: a CycloneDX vehicle SBOM (plus one per-module SBOM, the
  artifact each contract's `sbomRef` points at, **and** the transitive toolchain dependency
  SBOM) + a GSN assurance-case skeleton whose goals **defeat** when their evidence fails (the
  biased BMS swap defeats `G-bms` and the top-level safety goal). `loom sbom` emits the SBOMs
  standalone, without a sim run.
- **M6** — a higher-fidelity **Motoquant** plant (RK4 + thermal) that drops in via
  ...
  view), and **two distributed orchestrators** — **Compose/KUKSA** and **Eclipse Ankaios**
  (`loom deploy`) — sharing the same `Bus` interface and tick loop as the in-process path
  (verified equivalent against injected/fake clients; the live databroker path uses
  `docker-compose.yml`).
...
- **Orchestrator ↔ broker pairing.** An in-process broker can't be shared across containers,
  ...
  reusing the same tick loop. **Eclipse Ankaios** slots in as a third implementation
  (`AnkaiosOrchestrator` + a generated workload manifest, `loom deploy`), the automotive
  workload orchestrator that replaces Docker-Compose without touching the bus or the loop.
